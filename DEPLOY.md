...
## 2. 前端部署 (receipehelper.khtain.com)

### A. 修改生产环境配置
在项目根目录创建一个名为 `.env.production` 的文件，内容如下：
```ini
VITE_API_BASE_URL=https://receipehelperapi.khtain.com
```
...
### C. 上传到宝塔
1.  在宝塔面板 -> **网站** -> 添加站点 -> 域名填 `receipehelper.khtain.com`。
2.  进入该站点的根目录。
...
## 3. 后端部署 (receipehelperapi.khtain.com)
...
### C. 配置反向代理 (宝塔)
1.  在宝塔面板 -> **网站** -> 添加站点 -> 域名填 `receipehelperapi.khtain.com`。
...
1.  访问前端: `https://receipehelper.khtain.com`
2.  登录并观察顶部状态栏。
3.  如果状态栏全是绿色，说明前后端通信成功！
